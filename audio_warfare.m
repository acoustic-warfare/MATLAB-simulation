...
%sources = [audio_source(1900,2800,40,35,180,away_distance,0,3.5),audio_source(2000,2600,40,35,0,away_distance,0.5,4)];
%sources = [audio_source(4000,4400,50,30,0,away_distance,0,1)];
sources = [audio_source(3500,4500,40,30,180,away_distance,0,4),audio_source(3500,4500,40,30,0,away_distance,0,4)];
%sources = [audio_source(2800,3500,40,45,180,away_distance,0,1)];
%sources = [audio_source(6100,6200,40,35,0,away_distance,0,1.5)];
...
% [array_audio_signals(1).audio_signals,f_sampling] = load_data(); 
...
frequency_bands = linspace(3000,5000,f_bands_N);
          ...
          for freq_ind = 1:length(frequency_bands)      %Apply beamforming algo. in every freq. band
              ...
              if fps_rate > 0
                  for frame_ind = 2:length(frames_index(1,:))
                      ...
                      %color = sum(abs(fft(mic_data(frames_index(1,frame_ind):frames_index(2,frame_ind),1))).^2);
...
validation_locations = find_sources(xy_val_check,x_listen,y_listen,0.4);
...
frames_index_AW = [1;samples];
tic
audio_out = AW_listening(c,f_sampling,array_matrices,49,180,array_audio_signals);
toc
...
tic
f_bands_N = 55;          %Linearly spaced N bands,
frequency_bands = linspace(1000,4000,f_bands_N);
f_coefficients = get_filter_coefficients(f_sampling,frequency_bands);
audio_out2 = AW_listening_improved(c,f_sampling,array_matrices,49,180,array_audio_signals,f_coefficients,frequency_bands);
toc
%% Plot results 
figure(6)
...
tic
mega_coeffs = mega_filter_coefficients(array_matrices,f_sampling,frequency_bands,array_matrices(1),theta_pi,phi_pi,343);
topkek = AW_listening_new_improved(array_matrices,array_audio_signals,mega_coeffs,frequency_bands);
toc
